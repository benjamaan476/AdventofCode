...
size_t process(modules_t modules, bool part2)
{
    ...
    if (part2)
    {
        ...
        for (auto& node : modules.at(node_to_rx).in_nodes)
        { // find the counter nodes that run into the node that runs into rx
            cycles[node.first] = 0;
        }
    }
    ...
    for (int b = 0; b < limit; ++b)
    {
        ...
        if (part2)
        {
            ...
            {
                size_t s = 1;
                return std::accumulate(cycles.begin(), cycles.end(), s, [](size_t a, auto& b) {
                    return (size_t)std::lcm(a, b.second);
                                       });
            }
        }
    }
...
int main()
{
    auto test_values0 = load_input("in.txt");
    auto test_values1 = load_input("in.txt");
    auto actual_values = load_input("in.txt");
